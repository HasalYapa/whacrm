...
export async function GET() {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select(
        'provider, model, system_prompt, is_active, auto_reply_enabled, auto_reply_max_per_conversation, handoff_agent_id, booking_alert_phone, api_key, embeddings_api_key',
      )
...
export async function POST(request: Request) {
  try {
    ...
    // Booking-capture alert phone (optional): same "only when sent"
    // discipline so a partial save doesn't wipe it. Null clears it.
    if ('booking_alert_phone' in body) {
      const rawAlert = typeof body.booking_alert_phone === 'string'
        ? body.booking_alert_phone.trim()
        : ''
      shared.booking_alert_phone = rawAlert || null
    }
    if (rawEmbeddingsKey) {
